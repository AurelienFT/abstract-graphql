...
fn is_scalar(token: Token, scalars: Vec<scalars::Scalar>) -> bool {
    let basic_scalar = vec!["ID", "String", "Boolean", "Int", "Float"];
    let finded_in_scalar = scalars
        .iter()
        .find(|scalar| scalar.scalar_name.text == token.text);
    let finded_in_basic_scalar = basic_scalar.iter().find(|scalar| scalar == &&token.text);
    println!("token = {}", token.text);
    finded_in_scalar.is_some() || finded_in_basic_scalar.is_some()
}

fn extract_scope(tokens: Vec<Token>) -> Result<Vec<Token>, String> {
    if tokens.len() == 0 {
    ...
    let close_bracket = tokens
        .iter()
        .position(|token| token.ty == TokenType::CLOSE_BRACKET);
    match close_bracket {
        Some(bracket) => {
